...
% -----------------------------------------------------------------------------
% New rule: top tube (not necessarily horizontal) and seatstay both join the seat tube
% at a point 'seat_anchor_from_top_mm' distance from the 'visible top of the seat tube'; 
% the intersection point links with the seat tube length change.
%
% ===== geo (mm/deg) =====
%  Reach, Stack
%  head_angle_deg, head_tube_len
%  seat_angle_deg
%  seat_angle_mode     : 'horizontal'(default) | 'vertical' | 'auto'
%  seat_tube_dir       : 'backward'(default lean back) | 'forward'(lean forward)
%  seat_tube_len       : Base length of the visible seat tube
%  seat_tube_extend    : Only used to further extend the visible seat tube length (>=0)
%  seat_anchor_from_top_mm : Recess distance of the joint relative to the "visible top" 
%                          of the seat tube, default 200(mm)
%  chainstay, bb_drop
...
%
% Outputs out.points / out.geometry
% -----------------------------------------------------------------------------
...
% Old parameters that can be ignored (kept for compatibility but not used): top_tube_drop, seatstay_joint
...
% Head tube axis/normal, head tube bottom
u_axis_down = [cos(hta), -sin(hta)];
n_forward   = [sin(hta),  cos(hta)];
HB          = HTop + geo.head_tube_len * u_axis_down;

% Front Axle (incl. offset, y=0 enforced)
t_along = (HTop(2) + geo.fork_offset*cos(hta)) / sin(hta);
FA      = HTop + t_along*u_axis_down + geo.fork_offset*n_forward;
FA(2)   = 0;
A2C_implied = t_along;

% Seat tube direction: lean back / lean forward
switch lower(string(geo.seat_tube_dir))
    case "forward",  v_seat = [ cos(sta),  sin(sta)];
    otherwise,       v_seat = [-cos(sta),  sin(sta)]; % backward default
end

% -------- seat tube visible endpoint (fully controlled by your input) --------
seat_tube_len_draw = max(0, geo.seat_tube_len) + max(0, geo.seat_tube_extend);
ST_vis_top         = BB + seat_tube_len_draw * v_seat;

% -------- Unified Anchor Point: position 'seat_anchor_from_top_mm' from the seat tube top --------
% Convert "recess distance (mm)" into parameter length along the seat tube direction 
% (since v_seat is a unit vector, parameter = distance)
t_anchor = max(0, seat_tube_len_draw - max(0, geo.seat_anchor_from_top_mm));
SeatJoint = BB + t_anchor * v_seat;   % top tube & seatstay connect here

% -------- Remaining Geometry (unaffected by new anchor point) --------
% Trail: horizontal distance from the intersection of the steering axis and 
% the ground (y=-wheelR) to the ground contact point
...
% ETT: Defined by the industry standard "Effective Top Tube Length", calculated 
% as the horizontal distance between HTop's x-coordinate and SeatJoint's x-coordinate.
...
% top tube (no longer required to be horizontal): SeatJoint -> HeadTop
...
% Key Point
plot(SeatJoint(1), SeatJoint(2),'o','MarkerSize',5,'MarkerFaceColor',c_top,'MarkerEdgeColor','none'); % Unified Joint
plot(ST_vis_top(1), ST_vis_top(2),'s','MarkerSize',5,'MarkerFaceColor',c_seat,'MarkerEdgeColor','none'); % Seat Tube Top
